...
var preTestAnswerKeyUrl = 'https://mhalfmann.github.io/QualtricsTest/preTestAnswerKey.json';
...
function startPreTestFromArrays(configArray, answerKeyArray) {
    ...
    if (!configArray || !answerKeyArray || !Array.isArray(configArray) || !Array.isArray(answerKeyArray)) {
        document.getElementById('task-wrapper').innerHTML = 'Pre-Test: Konfiguration muss ein JSON-Array sein.';
        return;
    }
    if (configArray.length !== answerKeyArray.length) {
        document.getElementById('task-wrapper').innerHTML = 'Pre-Test: preTestConfig und preTestAnswerKey haben unterschiedliche Längen.';
        return;
    }
    if (i < 0 || i >= configArray.length) {
        document.getElementById('task-wrapper').innerHTML = 'Pre-Test: Index ' + n + ' ungültig (verfügbar: 1–' + count + ').';
        return;
    }
...
function renderStep3() {
    ...
    if (isVeelHulp) {
        ...
        document.getElementById('generate-tables-btn').onclick = function() {
            if (USE_PRETEST_RESTRICTIONS && !isSteps123Complete()) return;
...
function completeTask() {
    ...
    if (USE_PRETEST_RESTRICTIONS && !skipCompleteTaskValidation && (!isStep4PunnettComplete() || !isStep5FinalAnswersComplete())) return;
...
Qualtrics.SurveyEngine.addOnReady(function() {
    ...
    if (USE_PRETEST_RESTRICTIONS) {
        ...
        if (nextBtn && !nextBtn.dataset.pretestCollectBound) {
            nextBtn.dataset.pretestCollectBound = '1';
    ...
    Promise.all([
        fetch(preTestConfigUrl).then(function(r){ return r.json(); }),
        fetch(preTestAnswerKeyUrl).then(function(r){ return r.json(); })
    ]).then(function(values) {
        startPreTestFromArrays(values[0], values[1]);
    }).catch(function(e) {
        document.getElementById('task-wrapper').innerHTML = 'Fehler beim Laden der Pre-Test-Konfiguration: ' + e.message;
    });
});
